        ...
        while self.i < limit {
            match self.state {
                ...
                ParseState::Skip(remaining) => {
                    ...
                    self.state = NonZeroUsize::new(remaining.get() - skip).map(ParseState::Skip).unwrap_or(ParseState::Start);
                    break;
                }
                ParseState::Three => {
                    debug_assert_eq!(self.i, 0);
...
impl<R: std::io::BufRead + Clone> BitRead for BitReader<R> {
    ...
    fn skip(&mut self, bit_count: u32, name: &'static str) -> Result<(), BitReaderError> {
        self.reader.skip(bit_count).map_err(|e| BitReaderError::ReaderErrorFor(name, e))
    }
